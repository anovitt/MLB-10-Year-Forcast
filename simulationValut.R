...
totResults <- data.table()
for(i in 1:10){
  
  RESULTS <- data.table()

  NL <- c("ATL", "MIA", "NYM", "PHI", "WAS", "CHI", "CIN", "MIL", "PIT", "STL","ARI","COL","LAD","SDP","SFG")
  AL <- c("BAL", "BOS", "NYY", "TBR", "TBJ", "CWS", "CLE", "DET", "KCR", "MIN","HOU","LAA","OAA","SEA","TEX")
  teams <- c(NL, AL)
  league <- c(rep(1, length(NL)), rep(2, length(AL)))
  
  schedule <- rbind(make.schedule(NL, 6), 
                    make.schedule(AL, 6))
  
  # simulate talents 
  #talents <- rnorm(length(teams), 0, s.talent)  
  talents <- c(0.07,	-0.05,	0.01,	-0.01,	0,	0,	-0.03,	0.05,	-0.03,	0.05,	0,	-0.01,	0.07,	-0.05	,-0.01,
               -0.08,	0,	0.05,	0,	-0.05,	0,	0.04,	-0.04,	0	,0.04,	0.07,	-0.02,	0.05,	-0.02,	0.01)
  ...
  totResults<-rbind(totResults,RESULTS)
}  
  
totResults %>%
  group_by(Team) %>%
  summarise(win = sum(Wins)) %>%
  arrange(desc(win))

totResults %>%
  group_by(Team) %>%
  summarise(win = sum(Winner.WS)) %>%
  arrange(desc(win))
